...
You can use filters to change strings or manipulate a list of items.  To change the output of a Liquid object or variable, variables are written in double curly braces and filter are separated by a pipe character `|`, e.g. {% raw %} `{{ "/my/fancy/url" | append: ".html" }}` {% endraw %} 
...
You can also pass parameters to an include to change the content of a component based on the location in your website. For example, we want to add YouTube videos to the site and present them in a consistent way. We can put a placeholder for the YouTube video id.
